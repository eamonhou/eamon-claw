...
结合 Harness 驾驭工程的理念，我将 eamon-claw 的架构划分为四个核心层：
...
角色定位是“总指挥”与“协调者”。这是用户直接与之交互的应用程序。在我们的世界里：Claude Code 本身就是主机。它有 3 个核心职责：
...
角色定位是“能力提供方”，这是一个独立的程序，可以是本地脚本，也可以是远程服务。

它的核心职责是向主机暴露自己的能力。根据 MCP 规范，服务器可以提供三类核心能力：

- **工具**：这是最核心的能力，即 AI 可以执行的动作。例如，create_issue 就是一个工具。
- **资源**：这是 AI 可以引用的数据。例如，一个服务器可以暴露 @github:issue://123 这样的资源，让 AI 可以通过 @ 指令直接获取该 Issue 的上下文数据。
- **提示**：这是用户可以调用的工作流模板。服务器可以将一个复杂的、内部优化过的 Prompt，封装成一个简单的 Slash Command 供用户使用，例如 /mcp__github__list_prs。

#### 客户端（Client）

角色定位是“专线连接器”与“协议处理器”。这是一个由主机在内部创建和管理的组件，用户通常无感知。

### 过程

![alt text](image-20.png)

这个流程清晰地解释了 MCP 几个关键特性：
...
3. 命名空间：Claude Code 会自动为来自不同 MCP 服务器的工具加上命名空间前缀，格式为 mcp____，避免冲突。
