...
func NewPromptComposer(wd string, planMode bool) *PromptComposer {
	skillLoader := NewSkillLoader(wd)
...
func (p *PromptComposer) Build() schema.Message {
...
`)
	...
	agentsMdPath := filepath.Join(p.workDir, "AGENTS.md")
	...
	// 动态加载技能外挂（skills）
	skillsContent := p.skillLoader.LoadAll()
	if len(skillsContent) > 0 {
