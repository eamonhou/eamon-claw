...
type SubagentTool struct {
	...
	// 暂时用 interface 规避包循环依赖，底层通过断言使用
	reporter interface{}
}
...
func (t *SubagentTool) Definition() schema.ToolDefinition {
	return schema.ToolDefinition{
		Name:        t.Name(),
		Description: "派出一个专门用于深度搜索的子智能体。当你需要阅读大量代码、跨文件查找逻辑时请调用次工具。它在探索完毕后，会给你一份极度精炼的摘要报告。",
		InputSchema: schema.InputSchemaParams{
			"type": "object",
			"properties": schema.InputSchemaParams{
				"task_prompt": schema.InputSchemaParams{
					"type":        "string",
...
func (t *SubagentTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("子智能体执行失败：%w", err)
	}
