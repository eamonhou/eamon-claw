...
func (b *BenchmarkRunner) runSingleTest(ctx context.Context, tc TestCase) TestResult {
	...
	realProvider := provider.NewGeminiOpenAIProvider(b.modelName) // 使用真实的 GLM API
	session := history.NewSession(tc.ID, workDir)                 // 为本次跑分单独建一个 Session 记账
	trackedProvider := observer.NewCostTracker(realProvider, b.modelName, session)
	...
	err := eng.Run(ctx, session, nil, false)
	...
	if err != nil {
		return TestResult{
			TestCaseID:   tc.ID,
			Passed:       false,
			TotalCostCNY: session.TotalCost,
			DurationMs:   duration,
